...
namespace DESK_POP
{
    public partial class POP_WorkEndGDV : Form
    {
        ...
        private void POP_WorkEndGDV_Load(object sender, EventArgs e)
        {
            ...
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "설비명", "Equipment_Name", isVisible: false); //5
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "생성자", "Create_User_Name", isVisible: false); //6
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "공정코드", "Production_Operation_Code", isVisible: false); //7
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "공정명", "Operation_Name", isVisible: false); //8

            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "작업시작 예정일", "Start_Due_Date", isVisible: false); //9
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "작업상태", "Work_State_Name", isVisible: false); //10
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "작업시작일", "Start_Date", isVisible: false); //11
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "작업종료일", "Complete_Date", isVisible: false); //12
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "수정일", "Update_Time", isVisible: false); //13
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "수정 작업자 번호", "Update_User_No", isVisible: false); //14
            DataGridUtil.SetDataGridViewColumn_TextBox(dgvMain, "수정 작업자명", "Update_User_Name", isVisible: false); //15
        }
    }
}
